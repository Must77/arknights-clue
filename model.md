...
**过期效应（v5 仿真，N=10）**：全S* 约111条/年过期；利他策略约182条/年，是导致利他策略 cr/天 低于 S* 的原因之一（-0.7%）。
...
| 策略 | 名称 | 定义 |
|------|------|------|
| $\sigma_0$ | 完全利他 | 赠送所有线索，唯一例外：若该线索是凑齐7种缺失的最后一条（`all_clues[k]==1` 且当前集齐），则保留 |
| $\sigma_0^{\text{safe}}$ | 利他安全 | 同 $\sigma_0$，但当恰好缺2种且已持有其一时（$n_{\text{missing}} \leq 1$），也不赠出 |
| $\sigma_1$ | 完全利己 | 永不赠送；满仓时删除重复线索（+5信用/条）|
| $\sigma_1^{(7)}$ | 利己-7号变体 | 只赠7号线索的重复份；其余类型同 $\sigma_1$ 删除逻辑 |
| $\sigma^*$ | 最优个体策略 | 当 $c_{ik}^{\text{self}} + c_{ik}^{\text{recv}} \geq 2$ 时赠出 1 条类型 $k$（只赠多余，随机目标）|
| $\sigma^*_T$ | 定向最优策略 | 同 $\sigma^*$，但优先赠给缺少类型 $k$ 的好友 |
| $\sigma_{\theta=8}$ | 阈值-8策略 | 当自持总量 $\geq 8$ 且有重复时才赠出 |
| $\sigma_{\theta=10}$ | 满仓阈值策略 | 当自持总量 $\geq C_{\max}$（满仓）时赠出重复线索；若新入库线索非重复，则赠出库存中任意其他重复线索 |
...
$\sigma_1$（永不赠送）被 $\sigma^*$（赠重复）严格支配——$\sigma^*$ 在任何对手策略下均严格优于 $\sigma_1$（v5 仿真 E4 中所有偏离动机均为负，详见 FINDINGS.md §2.4）。
...
| D1 | 群体规模 | $N \in \{1, 2, 5, 10, 20, 50\}$（已仿真）；80/125 待超算 |
